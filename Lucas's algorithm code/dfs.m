...
% Output the parameters in a Python-readable format (e.g., as a .txt or .mat file)

fid = fopen('dfs_params.txt','w');
fprintf(fid, 'mapfile=%s\n', mapfile);
fprintf(fid, 'startlocation=[%d,%d]\n', startlocation(1), startlocation(2));
fprintf(fid, 'targetlocation=[%d,%d]\n', targetlocation(1), targetlocation(2));
fclose(fid);
